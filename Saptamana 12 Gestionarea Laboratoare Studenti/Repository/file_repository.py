...
def create_x_students(x, studenti, laboratoare):
    ...
    __assign_students(studenti,laboratoare)
def __assign_students(studenti,laboratoare):
    for i in range(1, len(studenti.get_all())+1):
        for j in range(1,len(laboratoare.get_all())+1):
...
def create_x_labs(x, laboratoare):
    ...
    for i in range(len(laboratoare.get_all()) + 1, len(laboratoare.get_all()) + x + 1):
