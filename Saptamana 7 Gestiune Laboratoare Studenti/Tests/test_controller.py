...
from Domain.Student import get_nume, create_student, get_grupa, get_student_by_id


def test_add_student():
    studenti = []
    add_student(studenti, 'nume', 1)
    assert(get_nume(studenti[0]) == 'nume')
    assert(get_grupa(studenti[0]) == 1)
    try:
...
def test_modify_student():
    ...
    assert(get_nume(studenti[0]) == 'new_nume')
    assert(get_grupa(studenti[0]) == 2)
